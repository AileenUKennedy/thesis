...
to make-turtles
  ...
  create-turtles number-nodes [

   ;; set turtles to random co-ordinates
...
show count turtles with [preinfectious?]
...
to setup-links
  let num-links (average-degree * number-nodes) / 2
  ...
  repeat 10
  [
    layout-spring turtles links 0.3 (world-width / (sqrt number-nodes)) 1
  ]
   ...
   ask link-neighbors with [susceptible?]
    ...
    ;;  show count turtles with [infected?]
     ...
     ;; set lambda (1 - ( 1 - p ) ^ num-infected )
       set lambda (1 - ( 1 - infect-prob) ^ num-infected )
     ;; show lambda
